    ...
    DYNAMIC_SECTION("SYCL platform " << platform.index() << ": " << platform.name()) {
      for (const auto &device : platform.devices()) {
        DYNAMIC_SECTION("SYCL device " << platform.index() << '.' << device.index() << ": " << device.name()) {
          SECTION("xtd::float16_t xtd::atan(xtd::float16_t)") {
            validate<xtd::float16_t, xtd::float16_t, xtd::atan, mpfr_atanf>(platform, device, ulps_half);
          }
          ...
          SECTION("xtd::float16_t xtd::atanf(xtd::float16_t)") {
            validate<xtd::float16_t, xtd::float16_t, xtd::atanf, mpfr_atanf>(platform, device, ulps_half);
          }
