      ...
      for (const auto &device : platform.devices()) {
        DYNAMIC_SECTION("SYCL device " << platform.index() << '.' << device.index() << ": " << device.name()) {
          SECTION("xtd::float16_t xtd::cos(xtd::float16_t)") {
            validate<xtd::float16_t, xtd::float16_t, xtd::cos, mpfr_cosf>(platform, device, ulps_half);
          }
          ...
          SECTION("xtd::float16_t xtd::cosf(xtd::float16_t)") {
            validate<xtd::float16_t, xtd::float16_t, xtd::cosf, mpfr_cosf>(platform, device, ulps_half);
          }
