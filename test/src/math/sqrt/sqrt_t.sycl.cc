      ...
      for (const auto &device : platform.devices()) {
        DYNAMIC_SECTION("SYCL device " << platform.index() << '.' << device.index() << ": " << device.name()) {
          SECTION("xtd::float16_t xtd::sqrt(xtd::float16_t)") {
            validate<xtd::float16_t, xtd::float16_t, xtd::sqrt, mpfr_sqrtf>(platform, device, ulps_half);
          }
          ...
          SECTION("xtd::float16_t xtd::sqrtf(xtd::float16_t)") {
            validate<xtd::float16_t, xtd::float16_t, xtd::sqrtf, mpfr_sqrtf>(platform, device, ulps_half);
          }
