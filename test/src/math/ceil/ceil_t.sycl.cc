      ...
      for (const auto &device : platform.devices()) {
        DYNAMIC_SECTION("SYCL device " << platform.index() << '.' << device.index() << ": " << device.name()) {
          SECTION("xtd::float16_t xtd::ceil(xtd::float16_t)") {
            validate<xtd::float16_t, xtd::float16_t, xtd::ceil, mpfr_ceilf>(platform, device, ulps_half);
          }
          ...
          SECTION("xtd::float16_t xtd::ceilf(xtd::float16_t)") {
            validate<xtd::float16_t, xtd::float16_t, xtd::ceilf, mpfr_ceilf>(platform, device, ulps_half);
          }
